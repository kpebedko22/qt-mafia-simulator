...
void Phase_Doctor::update_can_execute(QList<Player*> players_){
	...
	for (int i = 0; i < players_.count(); i++)
		if (players_[i]->role->is_doctor() && players_[i]->is_alive()) num_doctors++;

	if (num_doctors == 0){ can_execute = false; return;}
...
void Phase_Doctor::phase_1(QList<Player*> players_){
	...
	for (int i = 0; i < players_.count(); i++)
		if (players_[i]->role->is_doctor() && players_[i]->is_alive()){
...
void Phase_Doctor::phase_2(QList<Player*> players_){
	...
	log_text = "Доктор (" + choices[0]->voters[0]->get_name() + ")" + " выбрал " + choices[0]->elected->get_name() + "\n";
}
