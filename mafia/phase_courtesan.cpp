...
void Phase_Courtesan::update_can_execute(QList<Player*> players_){
	...
	for (int i = 0; i < players_.count(); i++)
		if (players_[i]->role->is_courtesan() && players_[i]->is_alive()) num_courtesan++;

	if (num_courtesan == 0){ can_execute = false; return;}
...
void Phase_Courtesan::phase_1(QList<Player*> players_){
	...
	for (int i = 0; i < players_.count(); i++)
		if (players_[i]->role->is_courtesan() && players_[i]->is_alive()){
...
void Phase_Courtesan::phase_2(QList<Player*> players_){
	...
	log_text = "Куртизанка (" + choices[0]->voters[0]->get_name() + ")" + " выбрала " + choices[0]->elected->get_name() + "\n";
}
