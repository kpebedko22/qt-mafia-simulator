...
void MainWindow::update_info_game() {
    ...
    ui->label_info_game->setText("Игроков: " + QString::number(current_game->get_num_players()) + \
                                 ...
                                 "\nСчет (Maf:Civ): " + QString::number(current_game->get_mafia_win()) + " : " +
                                 QString::number(current_game->get_civilians_win()));
    ...
    for (int i = 0; i < current_game->players.count(); i++) {
        ...
        if (!current_game->players[i]->role) string += "Роль: не назначена\n";
            ...
            if (current_game->players[i]->role->is(Role::Type::COURTESAN)) string += "Роль: куртизанка\n";
            if (current_game->players[i]->role->is(Role::Type::MAFIA)) string += "Роль: мафия\n";
            if (current_game->players[i]->role->is(Role::Type::CIVILIAN)) string += "Роль: мирный\n";
            if (current_game->players[i]->role->is(Role::Type::DETECTIVE)) string += "Роль: детектив\n";
            if (current_game->players[i]->role->is(Role::Type::DOCTOR)) string += "Роль: доктор\n";
        }
...
void MainWindow::make_labels_for_players_speech(int num_) {
    ...
    QList < QPoint * > coord;
    for (int i = 0; i < num_; i++)
        coord << new QPoint(static_cast<int>(430 * qCos(a * i)) + (this->width() / 2) - (this->width() / 7),
                            static_cast<int>(120 * qSin(a * i) + (this->height() / 2) - (this->height() / 8)));
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (rnd % 2 == 0) {
            if (names_male.count() > 0) {
                int rnd_male = QRandomGenerator::global()->bounded(static_cast<int>(0),
                                                                   static_cast<int>(names_male.count()));
                current_info.name = names_male[rnd_male];
                current_info.sex = 0;
                names_male.removeAt(rnd_male);
            } else {
                if (names_female.count() > 0) {
                    int rnd_female = QRandomGenerator::global()->bounded(static_cast<int>(0),
                                                                         static_cast<int>(names_female.count()));
