...
void Phase_Day::phase_1(QList<Player *> players_) {
    ...
    for (int i = 0; i < players_.count(); i++)
        if (players_[i]->is_alive()) {
            if (players_[i]->role->is(Role::Type::COURTESAN)) num_courtesans++;
            if (players_[i]->role->is(Role::Type::MAFIA)) num_mafia++;
            if (players_[i]->role->is(Role::Type::DOCTOR)) num_doctors++;
            if (players_[i]->role->is(Role::Type::DETECTIVE)) num_detectives++;
        }
    ...
    if (index_courtesan != -1) {
        courtesan_choice = choices[index_courtesan];
        log_text +=
                "Куртизанка " + courtesan_choice->voters[0]->get_name() + ": " + courtesan_choice->elected->get_name() +
                "\n";
    }
    if (index_mafia != -1) {
        mafia_choice = choices[index_mafia];
        log_text += "Мафия " + mafia_choice->voters[0]->get_name() + ": " + mafia_choice->elected->get_name() + "\n";
    }
    if (index_doctor != -1) {
        doctor_choice = choices[index_doctor];
        log_text += "Доктор " + doctor_choice->voters[0]->get_name() + ": " + doctor_choice->elected->get_name() + "\n";
    }
    if (index_detective != -1) {
        detective_choice = choices[index_detective];
        log_text +=
                "Детектив " + detective_choice->voters[0]->get_name() + ": " + detective_choice->elected->get_name() +
                "\n";
    }
    ...
    if (num_courtesans > 0)
        if (courtesan_choice->elected->role->is(Role::Type::MAFIA) && num_mafia == 1)
            mafia_choice_will_die = false;

    if (num_detectives > 0) {
        if (detective_choice->elected->role->is(Role::Type::MAFIA)) detective_choice_will_die = true;
        if (num_doctors > 0) if (detective_choice->elected == doctor_choice->elected) detective_choice_will_die = false;
        if (num_courtesans > 0)
            if (detective_choice->elected == courtesan_choice->elected ||
                courtesan_choice->elected->role->is(Role::Type::DETECTIVE))
                detective_choice_will_die = false;
    }

    if (num_courtesans > 0)
        if (courtesan_choice->elected->get_counter_courtesan() == 2)
            courtesan_choice_will_die = true;
...
void Phase_Day::phase_2(QList<Player *> players_) {
    ...
    for (int i = 0; i < players_.count(); i++)
        if (players_[i]->is_alive())
            choices << new Voting(players_[i]->role->dayChoice(players_[i], players_), players_[i], 1);
...
void Phase_Day::phase_3(QList<Player *> players_) {
    ...
    text += choices[0]->elected->get_name() + "т.к. за него проголосовало " + QString::number(choices[0]->votes) +
            " игроков. Прощай.";
    ...
    if (choices[0]->elected->role->is(Role::Type::MAFIA)) { text += " МАФИЕЙ"; }
    if (choices[0]->elected->role->is(Role::Type::CIVILIAN)) { text += " МИРНЫМ"; }
    if (choices[0]->elected->role->is(Role::Type::COURTESAN)) { text += " КУРТИЗАНКОЙ"; }
    if (choices[0]->elected->role->is(Role::Type::DOCTOR)) { text += " ДОКТОРОМ"; }
    if (choices[0]->elected->role->is(Role::Type::DETECTIVE)) { text += " ДЕТЕКТИВОМ"; }
