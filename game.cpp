...
void Game::update_numbers() {
    ...
    for (int i = 0; i < players.count(); i++)
        if (players[i]->is_alive()) {
            if (players[i]->role->is(Role::Type::COURTESAN)) num_courtesans++;
            if (players[i]->role->is(Role::Type::MAFIA)) num_mafia++;
            if (players[i]->role->is(Role::Type::DOCTOR)) num_doctors++;
            if (players[i]->role->is(Role::Type::DETECTIVE)) num_detectives++;
            if (players[i]->role->is(Role::Type::CIVILIAN)) num_civilians++;
        }
